    ...
    var settings = {}, roots = {}, caches = {},
        ...
        data = {
            ...
            nodeName: function (setting, node, newName) {
                ...
                var sysname="";
                if(node[setting.data.key.sysname]!==undefined){
                    sysname=node[setting.data.key.sysname];
                }
                var extname="";
                if(node[setting.data.key.extname]!==undefined){
                    extname="."+node[setting.data.key.extname];
                }
                return "" + node[key]+sysname+extname;
            },
        ...
        view = {
            ...
            makeNodeIcoStyle: function (setting, node) {
                ...
                if (!node.isAjaxing) {
                    var isParent = data.nodeIsParent(setting, node);
                    var iroot=setting.iconroot;
                    var icon = (isParent && node.iconOpen && node.iconClose) ? (node.open ? node.iconOpen : node.iconClose) : node[setting.data.key.icon];
                    if(node[setting.data.key.exticon]!==undefined){
                        var exticon=node[setting.data.key.exticon];
                        if (icon) icoStyle.push("background:url(",iroot+exticon,") center center no-repeat,url(",iroot+icon,") center center no-repeat;background-size:16px 16px;");
                    }else{
                        if (icon) icoStyle.push("background:url(",iroot+icon,") center center no-repeat;background-size:16px 16px;");
                    }
